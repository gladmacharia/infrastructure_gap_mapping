...
const layerStyles = {
    counties: { color: "#1f4e79", weight: 2, fillOpacity: 0.05 },
    constituencies: { color: "#2e75b6", weight: 1.5, fillOpacity: 0.08 },
    wards: { color: "#70ad47", weight: 1, fillOpacity: 0.05 },
    schools: { radius: 5, color: "#2e75b6", weight: 1, fillOpacity: 0.8 },
    hospitals: { radius: 6, color: "#c0392b", weight: 1, fillOpacity: 0.8 },
    police: { radius: 6, color: "#34495e", weight: 1, fillOpacity: 0.8 },
    roads: { color: "#666666", weight: 2, opacity: 0.8 },
    fiber: { color: "#8e44ad", weight: 3, opacity: 0.8 },
    electricity: { color: "#f39c12", weight: 3, opacity: 0.8 },
    analysis: { color: "#8e44ad", weight: 3, fillOpacity: 0.25, opacity: 0.9 }
};
...
async function loadLayer(layerName) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`${layerName} request failed with status ${response.status}`);
        }

        const geojson = await response.json();

        if (!geojson || geojson.type !== "FeatureCollection") {
            throw new Error(`${layerName} returned invalid GeoJSON`);
        }
    ...
    return L.geoJSON(geojson, {
        ...
        onEachFeature: function (feature, layer) {
            ...
            layer.on({
                ...
                mouseout: function () {
                    if (spatialLayers[layerName]) {
                        spatialLayers[layerName].resetStyle(layer);
                    }
                }
            });
        }
...
async function enableLayer(layerName) {
    ...
    if (layerButtons[layerName]) {
        layerButtons[layerName].checked = true;
    }
...
function disableLayer(layerName) {
    ...
    if (layerButtons[layerName]) {
        layerButtons[layerName].checked = false;
    }
...
map.on("moveend", function () {
    ...
    moveTimer = setTimeout(async function () {
        ...
        for (const layerName of layersToUpdate) {
            await loadLayer(layerName);
        }
...
async function loadDashboard() {
    try {
        ...
        if (facilityCanvas) {
            if (window.facilityChart) {
                window.facilityChart.destroy();
            }
                    ...
                    datasets: [
                        {
                            ...
                            data: [data.hospitals, data.schools, data.police]
                        }
                    ]
                },
                options: {
                    ...
                    datasets: [
                        {
                            ...
                            data: [data.roads, data.electricity, data.fiber]
                        }
                    ]
                },
                options: {
...
function handleAIResult(result) {
    ...
    if (spatialLayers.analysis) {
        delete spatialLayers.analysis;
    }
...
suggestionButtons.forEach(button => {
    button.addEventListener("click", function () {
        askAIQuestion(button.textContent.trim());
    });
});
...
async function performSearch() {
    ...
    try {
        ...
        if (!geojson || geojson.type !== "FeatureCollection") {
            throw new Error("Search returned invalid GeoJSON");
        }
        ...
        searchResultLayer = L.geoJSON(geojson, {
            ...
            onEachFeature: function (feature, layer) {
                ...
                layer.bindPopup(`
                    ...
                    Type: ${properties.location_type}<br>
                    ${properties.county_name
                        ? `County: ${properties.county_name}<br>`
                        : ""}
                    ${properties.constituency_name
                        ? `Constituency: ${properties.constituency_name}<br>`
                        : ""}
